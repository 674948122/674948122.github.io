import{_ as r,Q as a,a2 as s,X as n,a5 as e,a3 as p,a4 as o,E as c}from"./framework-a091056b.js";const d={},i=o(`<h1 id="一些思路" tabindex="-1"><a class="header-anchor" href="#一些思路" aria-hidden="true">#</a> 一些思路</h1><p>----------------------------------------------------------1、微前端功能点答疑------------------------------------------------------------------</p><p>1、一般情况下不需要采用微前端模式的情况下，架构部推出的前端框架标准配置应该是什么样的</p><pre><code>a、qiankun的依赖、配置项（registerMicroApps、start等函数）、挂载点、读取子应用的配置或者请求保留是否存在问题
...
</code></pre><p>-------------------------------------------------------------2、子应用管理中心设计----------------------------------------------------------------------------------------</p><p>关键元素：</p>`,24),_={href:"http://tunny.com:8080",target:"_blank",rel:"noopener noreferrer"},l={href:"https://qiankun.umijs.org/guide/",target:"_blank",rel:"noopener noreferrer"},u=o("<p>5、子应用图标 6、子应用描述 7、主应用路由匹配前缀 8、开发者 9、更新时间 10、子应用状态（启用、停用）</p><p>11、子应用路由清单（类似做一个菜单管理）</p><p>tips：子应用只能创建一个一级菜单</p><p>菜单类型一旦创建，不许修改（目录，菜单，按钮，子应用）</p><p>------------------------------------------------------------3、梳理迁移后不可用的接口列表按照模块说明----------------------------------------------------------------------</p><p>------------------------------------------------------------4、菜单模块对应修改说明-------------------------------------------------------------------------------------------</p><p>1、新增菜单时</p><p>菜单类型增加可选项（目录、菜单、按钮、子应用）</p><p>2、点击子应用时，加载子应用菜单列表</p><p>3、用户可将加载出的子应用菜单列表勾选</p><p>4、用户点击保存会将勾选的子应用菜单追加到主应用菜单树下面</p><p>5、在维护角色菜单权限的时候正常操作即可</p>",12);function h(m,f){const t=c("ExternalLinkIcon");return a(),s("div",null,[i,n("p",null,[e("1、子应用中文名称（唯一） 2、子应用英文名称（唯一 ，小写 中划线 长度不超过10位不允许特殊符号和数字） 3、挂载节点的Dom 唯一标识ID（一般情况下，domId为 主应用的 内容区域ID） 4、子应用入口（此内容一般为nginx部署之后的启动地址，比如 "),n("a",_,[e("http://tunny.com:8080"),p(t)]),e("） -------(是否带http不确定，自动补全不确定) 配置为字符串时，表示微应用的访问地址，例如 "),n("a",l,[e("https://qiankun.umijs.org/guide/"),p(t)]),e(" 需要填写完整http协议 尾部以/结尾，或者/index.html结尾")]),u])}const k=r(d,[["render",h],["__file","一些思路.html.vue"]]);export{k as default};
